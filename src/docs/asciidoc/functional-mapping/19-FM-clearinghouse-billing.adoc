...
TODO: We must determine if there is any possibility of this happening before including this in a final document, determine if we must develop/specify clearing holuse functionality and, if not, reconcile it with whatever model an external clearing house might use. I have no idea if it's possible to 'buy' clearing house services from a financial institution but I would guess at 'no chance at all'.
